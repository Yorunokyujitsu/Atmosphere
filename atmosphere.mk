...
dist-no-debug: package3 $(CURRENT_DIRECTORY)/$(ATMOSPHERE_OUT_DIR)
	...
	cp -r config_templates/hosts/dns.txt $(DIST_DIR)/atmosphere/hosts/emummc.txt
	cp -r config_templates/hosts/dns.txt $(DIST_DIR)/atmosphere/hosts/sysdns.txt
	cp -r config_templates/warmboot_mariko $(DIST_DIR)/warmboot_mariko
	...
	cp troposphere/haze/haze.nro $(DIST_DIR)/switch/Haze/haze.nro
	cp troposphere/reboot_to_payload/reboot_to_payload.nro $(DIST_DIR)/switch/Reboot2payload/reboot_to_payload.nro
...
package3: emummc fusee stratosphere mesosphere exosphere troposphere
	@if [ -f fusee/loader.kip ]; then \
	    cp fusee/loader.kip stratosphere/loader/$(ATMOSPHERE_OUT_DIR)/loader.kip; \
	fi
