...
namespace dbk {
    ...
    namespace {
        ...
        bool g_create_20x_downgrade_flag = false;
        bool g_create_19x_downgrade_flag = false;

        enum class DowngradePatchType {
            Firmware20x,
            Firmware19x,
        };

        bool CreateDowngradeFlag(DowngradePatchType type) {
            u64 is_emummc;

            if (R_FAILED(splGetConfig(
                    static_cast<SplConfigItem>(ExosphereEmummcType),
                    &is_emummc))) {
                return false;
            }
            ...
            switch (type) {
                case DowngradePatchType::Firmware20x:
                    path = is_emummc ? "sdmc:/backup/configs/flags/dg_emummc.flag" : "sdmc:/backup/configs/flags/dg_emmc.flag";
                    break;

                case DowngradePatchType::Firmware19x:
                    path = is_emummc ? "sdmc:/backup/configs/flags/dg_19_emummc.flag" : "sdmc:/backup/configs/flags/dg_19_emmc.flag";
                    break;
            }
    ...
    ErrorMenu::ErrorMenu(const char *text, const char *subtext, Result rc) : AlertMenu(nullptr, text, subtext, rc)  {
        ...
        /* Add buttons. */
        this->AddButton(ExitButtonId, "나가기", x + HorizontalInset, button_y, button_width, ButtonHeight);
    ...
    WarningMenu::WarningMenu(std::shared_ptr<Menu> prev_menu, std::shared_ptr<Menu> next_menu, const char *text, const char *subtext, Result rc) : AlertMenu(prev_menu, text, subtext, rc), m_next_menu(next_menu) {
        ...
        this->AddButton(BackButtonId, "뒤로", x + HorizontalInset, button_y, button_width, ButtonHeight);
        this->AddButton(ContinueButtonId, "계속", x + HorizontalInset + button_width + ButtonHorizontalGap, button_y, button_width, ButtonHeight);
    ...
    MainMenu::MainMenu() : Menu(nullptr) {
        ...
        this->AddButton(InstallButtonId, "설치", x + HorizontalInset, y + TitleGap, WindowWidth - HorizontalInset * 2, ButtonHeight);
        this->AddButton(ExitButtonId, "나가기", x + HorizontalInset, y + TitleGap + ButtonHeight + VerticalGap, WindowWidth - HorizontalInset * 2, ButtonHeight);
            ...
            switch (activated_button->id) {
                case InstallButtonId:
                {
                    ...
                    if (R_FAILED(rc = splGetConfig(SplConfigItem_HardwareType, &hardware_type))) {
                        ChangeMenu(std::make_shared<ErrorMenu>("오류가 발생했습니다", "하드웨어 정보 취득 실패.", rc));
                        return;
                    }

                    if (R_FAILED(rc = splGetConfig(static_cast<SplConfigItem>(ExosphereHasRcmBugPatch), &has_rcm_bug_patch))) {
                        ChangeMenu(std::make_shared<ErrorMenu>("오류가 발생했습니다", "RCM 취약점 상태 확인 실패.", rc));
                        return;
                    }

                    if (R_FAILED(rc = splGetConfig(static_cast<SplConfigItem>(ExosphereEmummcType), &is_emummc))) {
                        ChangeMenu(std::make_shared<ErrorMenu>("오류가 발생했습니다", "emuMMC 상태 확인 실패.", rc));
                        return;
                    }
                    ...
                    if (is_erista && has_rcm_bug_patch && !is_emummc) {
                        ChangeMenu(std::make_shared<WarningMenu>(g_current_menu, file_menu, "경고: 패치된 기기 감지됨", "퓨즈가 손상되거나 스위치가 작동 불능이 될 수 있습니다."));
                    } else {
    ...
    void FileMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "펌웨어 폴더 선택", x, y, WindowWidth, WindowHeight);
    ...
    ValidateUpdateMenu::ValidateUpdateMenu(std::shared_ptr<Menu> prev_menu) : Menu(prev_menu), m_has_drawn(false), m_has_info(false), m_has_validated(false) {
        ...
        /* Add buttons. */
        this->AddButton(BackButtonId, "뒤로", x + HorizontalInset, y + WindowHeight - BottomInset - ButtonHeight, button_width, ButtonHeight);
        this->AddButton(ContinueButtonId, "계속", x + HorizontalInset + button_width + ButtonHorizontalGap, y + WindowHeight - BottomInset - ButtonHeight, button_width, ButtonHeight);
        ...
        if (R_FAILED(this->GetUpdateInformation())) {
            ...
            this->LogText("펌웨어 검증 중, 잠시만 기다려주세요...\n");
        }
    }

    Result ValidateUpdateMenu::GetUpdateInformation() {
        Result rc = 0;
        this->LogText("경로: sdmc:%s\n", g_update_path);

        /* Attempt to get the update information. */
        if (R_FAILED(rc = amssuGetUpdateInformation(&m_update_info, g_update_path))) {
            if (rc == 0x1a405) {
                this->LogText("폴더에서 펌웨어 파일을 찾을 수 없습니다.\nNCA 파일명이 올바른지 확인하세요!\n결과: 0x%08x\n", rc);
            } else {
                this->LogText("업데이트 정보를 취득할 수 없습니다.\n결과: 0x%08x\n", rc);
            }
            return rc;
        }

        const u32 target_major = (m_update_info.version >> 26) & 0x1F;
        const u32 target_minor = (m_update_info.version >> 20) & 0x1F;
        const u32 target_micro = (m_update_info.version >> 16) & 0x0F;

        const u32 target_version = EncodeVersion(target_major, target_minor, target_micro);

        // 21.0.0+ > 20.0.0 through 20.5.0
        g_create_20x_downgrade_flag = hosversionAtLeast(21, 0, 0) && target_version >= EncodeVersion(20, 0, 0) &&   target_version <= EncodeVersion(20, 5, 0);

        // 20.0.0+ > 19.0.1 or lower
        g_create_19x_downgrade_flag = hosversionAtLeast(20, 0, 0) && target_version <= EncodeVersion(19, 0, 1);

        /* Print update information. */
        this->LogText("- 버전: %d.%d.%d\n", target_major, target_minor, target_micro);
        if (m_update_info.exfat_supported) {
            this->LogText("- exFAT: 지원\n");
        } else {
            this->LogText("- exFAT: 미지원\n");
        }
        this->LogText("- 펌웨어 검증: %d\n", m_update_info.num_firmware_variations);
            ...
            switch (activated_button->id) {
                ...
                case ContinueButtonId:
                    ...
                    if (m_update_info.exfat_supported && R_FAILED(m_validation_info.exfat_result)) {
                        next_menu = std::make_shared<WarningMenu>(g_current_menu, next_menu, "경고: exFAT 펌웨어가 없거나 손상됨", "계속 진행하시겠습니까?");
                    }
                    ...
                    if (EncodeVersion((version >> 26) & 0x1f, (version >> 20) & 0x1f, (version >> 16) & 0xf) > g_supported_version) {
                        next_menu = std::make_shared<WarningMenu>(g_current_menu, next_menu, "경고: 너무 새로운 펌웨어로, 지원 여부 확인이 불가합니다", "계속 진행하시겠습니까?");
                    }
    ...
    void ValidateUpdateMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "업데이트 정보", x, y, WindowWidth, WindowHeight);
    ...
    ChooseResetMenu::ChooseResetMenu(std::shared_ptr<Menu> prev_menu) : Menu(prev_menu) {
        ...
        /* Add buttons. */
        this->AddButton(ResetToFactorySettingsButtonId, "공장 초기화", x + HorizontalInset, y + TitleGap, button_width, ButtonHeight);
        this->AddButton(PreserveSettingsButtonId, "기존 설정 유지", x + HorizontalInset + button_width + ButtonHorizontalGap, y + TitleGap, button_width, ButtonHeight);
    ...
    void ChooseResetMenu::Update(u64 ns) {
        ...
        if (const Button *activated_button = this->GetActivatedButton(); activated_button != nullptr) {
            ...
            next_menu = std::make_shared<WarningMenu>(g_current_menu, std::make_shared<InstallUpdateMenu>(g_current_menu), "업데이트 설치가 준비되었습니다", "계속 진행하시겠습니까?");

            if (g_reset_to_factory) {
                ChangeMenu(std::make_shared<WarningMenu>(g_current_menu, next_menu, "경고: 공장 초기화 선택됨", "세이브 데이터 및 설치된 게임이 영구적으로 삭제됩니다."));
            } else {
                ChangeMenu(next_menu);
            }
        }
    ...
    void ChooseResetMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "설정 모드 선택", x, y, WindowWidth, WindowHeight);
        this->DrawButtons(vg, ns);
    }
    ...
    ChooseExfatMenu::ChooseExfatMenu(std::shared_ptr<Menu> prev_menu) : Menu(prev_menu) {
        ...
        /* Add buttons. */
        this->AddButton(Fat32ButtonId, "설치 (FAT32)", x + HorizontalInset, y + TitleGap, button_width, ButtonHeight);
        this->AddButton(ExFatButtonId, "설치 (FAT32 + exFAT)", x + HorizontalInset + button_width + ButtonHorizontalGap, y + TitleGap, button_width, ButtonHeight);
    ...
    void ChooseExfatMenu::Update(u64 ns) {
        ...
        if (const Button *activated_button = this->GetActivatedButton(); activated_button != nullptr) {
            ...
            ChangeMenu(std::make_shared<WarningMenu>(g_current_menu, std::make_shared<InstallUpdateMenu>(g_current_menu), "업데이트 설치가 준비되었습니다", "계속 진행하시겠습니까?"));
        }
    ...
    void ChooseExfatMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "드라이버 종류 선택", x, y, WindowWidth, WindowHeight);
        this->DrawButtons(vg, ns);
    }
    ...
    InstallUpdateMenu::InstallUpdateMenu(std::shared_ptr<Menu> prev_menu) : Menu(prev_menu), m_install_state(InstallState::NeedsDraw), m_progress_percent(0.0f) {
        ...
        /* Add buttons. */
        this->AddButton(ShutdownButtonId, "종료", x + HorizontalInset, y + WindowHeight - BottomInset - ButtonHeight, button_width, ButtonHeight);
        this->AddButton(RebootButtonId, "재부팅", x + HorizontalInset + button_width + ButtonHorizontalGap, y + WindowHeight - BottomInset - ButtonHeight, button_width, ButtonHeight);
    ...
    Result InstallUpdateMenu::TransitionUpdateState() {
        ...
        if (m_install_state == InstallState::NeedsSetup) {
            ...
            if (R_FAILED(rc = amssuSetupUpdate(nullptr, UpdateTaskBufferSize, g_update_path, g_use_exfat))) {
                this->LogText("업데이트 설정 실패.\n결과: 0x%08x\n", rc);
                this->MarkForReboot();
                return rc;
            }

            /* Log setup completion. */
            this->LogText("업데이트 설정 완료.\n");
            ...
            if (R_FAILED(rc = amssuRequestPrepareUpdate(&m_prepare_result))) {
                this->LogText("업데이트 준비 요청 실패.\n결과: 0x%08x\n", rc);
                this->MarkForReboot();
                return rc;
            }

            /* Log awaiting prepare. */
            this->LogText("업데이트 준비 중...\n");
            ...
            if (R_FAILED(rc = amssuApplyPreparedUpdate())) {
                this->LogText("업데이트 적용 실패.\n결과: 0x%08x\n", rc);
            } else {
                /* Log success. */
                this->LogText("업데이트 적용 완료.\n");

                if (g_create_20x_downgrade_flag) {
                    CreateDowngradeFlag(DowngradePatchType::Firmware20x);
                }

                if (g_create_19x_downgrade_flag) {
                    CreateDowngradeFlag(DowngradePatchType::Firmware19x);
                }
                    ...
                    if (R_FAILED(rc = nsResetToFactorySettingsForRefurbishment())) {
                        ...
                        if (rc == MAKERESULT(Module_Libnx, LibnxError_IncompatSysVer)) {
                            if (R_FAILED(rc = nsResetToFactorySettings())) {
                                this->LogText("공장 초기화 실패.\n결과: 0x%08x\n", rc);
                                this->MarkForReboot();
                                return rc;
                            }
                        } else {
                            this->LogText("리퍼비시용 공장 초기화 실패.\n결과: 0x%08x\n", rc);
                            this->MarkForReboot();
                            return rc;
                        }
                    }

                    this->LogText("공장 초기화 완료.\n", rc);
                }
            }
    ...
    void InstallUpdateMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "업데이트 설치 중", x, y, WindowWidth, WindowHeight);
    ...
    bool InitializeMenu(u32 screen_width, u32 screen_height) {
        ...
        if (R_FAILED(rc = splGetConfig(static_cast<SplConfigItem>(ExosphereApiVersionConfigItem), &version))) {
            ChangeMenu(std::make_shared<ErrorMenu>("Atmosphere를 찾을 수 없음", "Daybreak를 사용하려면 Atmosphere가 필요합니다.", rc));
            return false;
        }
        ...
        if (!ams_supports_sysupdate_api) {
            ChangeMenu(std::make_shared<ErrorMenu>("오래된 Atmosphere 버전", "Daybreak는 Atmosphere 0.14.0 이상에서 작동합니다.", rc));
            return false;
        }

        /* Ensure DayBreak is ran as a NRO. */
        if (envIsNso()) {
            ChangeMenu(std::make_shared<ErrorMenu>("지원되지 않는 실행 환경", "Daybreak를 Sphaira에서 실행하세요.", rc));
            return false;
        }
